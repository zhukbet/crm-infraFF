...
- Головний важіль масштабування — більше реплік `workers` і чергами (BullMQ/Redis), не зміна
  платформи.
- **[`DEPLOY.md`](./DEPLOY.md)** — конкретний покроковий гайд для цього шляху: сервер, `.env`,
  перший запуск, прив'язка Telegram-вебхука, перший агент (allowlist), оновлення. Реального
  живого деплою в цій сесії немає — хостинг-креденшелів немає — але кожен компонент (образи,
  healthcheck, graceful shutdown) уже перевірений через docker-compose smoke-test у CI.
